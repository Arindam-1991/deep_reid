...
class ClassMemoryLoss(Module):
    def __init__(self, matcher, num_classes, mem_batch_size=16):
        """
        Inputs:
        ...
        self.height = matcher.height
        self.width = matcher.width
        self.mem_batch_size = mem_batch_size
        self.matcher = matcher
        self.register_buffer('class_memory', torch.zeros(num_classes, matcher.num_features,  matcher.height, matcher.width))
    ...
    def forward(self, feature, target):
        ...
        return torch.mean(loss), torch.mean(acc)
